...
- [x] **S4-T7** PR4b **VISUAL QA GATE (BEH-4 — not a code task)**: manual dark-mode QA — main views (tables, dialogs, forms, toasts) vs pre-migration: surfaces, text, borders, gold/lavender accents; CSS grid shell (D3) + AuraCompat 14px density under 16px root (D1) acceptance. **Sign-off REQUIRED before slice 5.** Deps: S4-T6. Verify: QA record committed. — DONE 2026-08-19: maintainer sign-off "si muchisimo mejor" after dark-scheme fix cf6f4bb (`darkModeSelector: '.dark-mode'` class selector — bare 'html' produced invalid nested CSS and the dark scheme never applied; text tokens fell to light slate #334155).
- [x] **S4-T8** PR4b gate: `npm test` green + QA sign-off recorded. — DONE: `npm test` green (59 files / 546 tests) + S4-T7 sign-off recorded above.
