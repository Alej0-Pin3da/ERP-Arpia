...
- **Verificación:** `py_compile` OK, `configure_mappers` OK, `alembic heads` único `0043`, `npm run build` OK (3.20s), `npx vitest run` 41/41. **DB resuelta 2026-09-23:** Postgres docker healthy, `alembic upgrade head` aplicado en dev (0043), `pytest test_cotizaciones_api.py` 15/15 + vecinos omisiones/ventas 49/49, ruff check+format OK. (El 405 inicial era bug en los tests — pegaban a `/{id}` sin `/estado`, ya corregido en `b71e493`.)
