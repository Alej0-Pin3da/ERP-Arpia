...
# inventario) SI suman.
FECHA_CORTE_SNAPSHOT = datetime(2025, 10, 25, tzinfo=UTC).date()
...
def _identidades_bom_db(db) -> dict[str, int]:
    ...
    for _bom, producto, insumo in filas:
        conteos[f"{clave_normalizada(producto)}|{clave_normalizada(insumo)}"] += 1
    return conteos
...
def _identidades_compras_db(db) -> dict[str, int]:
    ...
    conteos: dict[str, int] = defaultdict(int)
    filas = db.query(Insumo.nombre).join(CompraInsumo, CompraInsumo.insumo_id == Insumo.id).all()
    for (insumo,) in filas:
...
def _identidades_ventas_db(db) -> dict[str, int]:
    ...
    conteos: dict[str, int] = defaultdict(int)
    filas = (
        db.query(Producto.nombre).join(DetalleVenta, DetalleVenta.producto_id == Producto.id).all()
    )
...
def _n7b_stock_negativo(db, plan: PlanValidacion) -> CheckResult:
    negativos = [
        i for i in db.query(Insumo).all() if i.stock_actual is not None and i.stock_actual < 0
    ]
    if negativos:
...
def _n7c_finanzas(db, plan: PlanValidacion) -> CheckResult:
    ...
    situacion = (
        "; ".join(errores)
        if errores
        else ("socios ausentes (F6 no corrida)" if faltante else "finanzas coherentes con el plan")
    )
    if notas:
...
def _n7d_cuadre(db, plan: PlanValidacion) -> CheckResult:
    ...
    db_insumos = {clave_normalizada(i.nombre): i for i in db.query(Insumo).all()}
    ...
    presentes = [item for item in snapshot if clave_normalizada(item.insumo_nombre) in db_insumos]
    ausentes = [
        item for item in snapshot if clave_normalizada(item.insumo_nombre) not in db_insumos
    ]
    if ausentes:
        if presentes:
            return CheckResult(
        ...
        return CheckResult(
            "N7d",
            "WARN",
            f"{len(ausentes)} insumos del snapshot ausentes del catalogo (F4 OCT25 no aplicada)",
        )
    ...
    if divergencias:
        return CheckResult("N7d", "WARN", "cuadre divergente: " + "; ".join(divergencias[:5]))
    ...
    for venta in db.query(Venta).all():
        for detalle in venta.detalles:
            clave = (
                ...
                clave_normalizada(detalle.variante.nombre_variante) if detalle.variante else None,
                detalle.cantidad,
            ...
            if venta.descuento_porcentaje > 0:
                errores.append(
                    f"{detalle.producto.nombre}: descuento_porcentaje > 0 (doble descuento, VTA-2)"
                )
            elif _moneda(venta.total_venta) != _moneda(detalle.cantidad * plan_linea.precio):
                errores.append(
...
def _fecha_real_ok(fecha, ahora) -> bool:
    ...
    aware = fecha if fecha.tzinfo else fecha.replace(tzinfo=UTC)
...
def _n7f_fechas(db, plan: PlanValidacion) -> CheckResult:
    ...
    if falsos:
        return CheckResult("N7f", "ERROR", f"fechas falsas now() en: {', '.join(falsos[:5])}")
