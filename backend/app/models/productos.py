...
class Producto(Base):
    ...
    bom_insumos: Mapped[list[BomInsumo]] = relationship(
        back_populates="producto", lazy="selectin"
    )
...
class VarianteProducto(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "producto_id", "nombre_variante", name="uq_variantes_producto_nombre"
        ),
    )
...
class BomInsumo(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "producto_id", "insumo_id", "variante_id", name="uq_bom_insumos_combo"
        ),
    )
    ...
    insumo: Mapped[Insumo] = relationship(lazy="selectin")
...
class BomProducto(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "combo_id", "producto_incluido_id", name="uq_bom_productos_combo"
        ),
    )
    ...
    combo: Mapped[Producto] = relationship(
        foreign_keys=[combo_id], backref="combo_items"
    )
    producto_incluido: Mapped[Producto] = relationship(
        foreign_keys=[producto_incluido_id]
    )
